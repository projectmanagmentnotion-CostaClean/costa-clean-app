...
- CP-2B is `BLOCKED_PENDING_EXPLICIT_V4_AUTHORIZATION`. A future prompt must name the exact clean CP-2A.3 commit, accept [client-portal/CP2B_EXACT_QA_AUTHORIZATION_V4.md](client-portal/CP2B_EXACT_QA_AUTHORIZATION_V4.md), verify `scripts/client-portal/cp2b_qa_package_v4.manifest.json`, and use a new private backup bound to that HEAD.
