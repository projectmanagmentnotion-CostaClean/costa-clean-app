...
- Resultado de lectura relacionado: ver `docs/JOB_LINES_APPDATA_TO_EDITOR_FIX_QA.md`. A fecha 2026-07-01 se confirmo un caso real donde `jobs` si llegaba por REST pero `job_lines` no era visible con bearer `anon`, de modo que el editor caia en `billing_lines: []` hasta cambiar la lectura a `access_token` de sesion y exponer debug visible.
- Resultado DB/RLS posterior: ver `docs/JOB_LINES_RLS_SELECT_FIX_QA.md`. A fecha 2026-07-01 el siguiente cuello ya no era frontend sino visibilidad de `job_lines` con `authMode = session`, `status = 200` y `rawCount = 0`, compatible con policy/RLS pendiente en la base real.
...
- Resultado line-summary relacionado: ver `docs/JOB_LINES_NO_SUMMARY_FIX_QA.md`. A fecha 2026-07-01 el concepto principal del servicio deja de incrustar `(+N linea(s))` cuando existen `billing_lines`, y los prefills/facturacion vuelven a priorizar las lineas reales.
- Resultado editor/load-path relacionado: ver `docs/JOB_EDIT_REAL_LINES_FIX_QA.md`. A fecha 2026-07-01 el editor de servicio ya no descarta `billing_lines` reales cuando PostgREST entrega numericos como texto, y vuelve a abrir con todos los bloques editables.
- Resultado form-state final relacionado: ver `docs/JOB_FORM_STATE_LINES_FINAL_FIX_QA.md`. A fecha 2026-07-01 la ruta editable de servicios ya tiene helper unico, separacion clara entre display y formulario, y logging DEV del conteo real hasta `p_lines`.
- Resultado audit/debug visible relacionado: ver `docs/JOB_LINES_DEBUG_PANEL_NOT_VISIBLE_AUDIT.md`. A fecha 2026-07-01 quedo confirmado que el editor real visible es `JobDetailCard` dentro de `MajorEditFlowOverlay`, y que el panel debug no se veia porque no habia sido publicado en el build que el usuario estaba probando.
- Resultado UX live posterior: ver `docs/JOB_EDITOR_LIVE_SAVE_UX_QA.md`. A fecha 2026-07-01 el editor de servicio ya mantiene estado optimista local tras guardar y no depende de que el refresh global traiga el `job` fresco al primer intento.
- Resultado toasts + factura fiscal posterior: ver `docs/GLOBAL_TOASTS_AND_INVOICE_FISCAL_QA.md`. A fecha 2026-07-01 la app ya tiene infraestructura global de avisos y el documento de factura vuelve a leer el snapshot fiscal para mostrar `NIF/CIF` con etiqueta correcta.
