...
| Crear servicio | Servicios | `src/features/jobs/JobCreateFlow.tsx`, `JobCreateForm.tsx`, `src/pages/JobsPage.tsx` | Sprint 10 confirma que el alta de servicio ya necesita StepFlow real por sus relaciones con contexto, agenda, base de cobro y duplicados. La mejora segura no es crear un QuickFlow paralelo, sino usar el StepFlow oficial como alta principal y dejar el modulo como launcher rapido. | medio | Completado en Sprint 10: se mantiene `FullscreenStepFlow` como alta oficial, se refuerza el launcher rapido desde `JobsPage` y se mejora la lectura del workspace sin tocar el write path | Sprint 10 completado |
...
| Registrar cobro | Cobros | `src/features/payments/PaymentCreateFlow.tsx`, `PaymentCreateForm.tsx` | Flujo mas corto, pero aun con duplicados, confirmacion y relacion obligatoria con factura. Sprint 11 decide no migrarlo todavia: se pule el workspace y se mantiene el create actual por seguridad. | medio | Mantener flujo actual, documentar contrato StepFlow futuro y reevaluar solo cuando el write path financiero este suficientemente aislado | Seguimiento post Sprint 11 |
| Crear gasto | Gastos | `src/features/expenses/ExpenseCreateFlow.tsx`, `ExpenseCreateForm.tsx` | Flujo largo con soporte documental, lectura fiscal y duplicados. Sprint 11 mejora la capa de workspace pero pospone la migracion para no mezclar UX con riesgo fiscal/persistencia. | medio | Mantener create actual y preparar una futura migracion por pasos cuando soporte, revision y guardado puedan probarse de forma aislada | Seguimiento post Sprint 11 |
| Editar gasto | Gastos | `src/features/expenses/ExpenseEditFlow.tsx` | Mismo problema de create: flujo largo con logica propia y varias consecuencias fiscales visibles. | medio | Reusar el contrato de pasos y estados solo en un sprint especifico de migracion segura | Seguimiento post Sprint 11 |
...
| Snapshot y paquete fiscal | Cierre fiscal | `src/pages/FiscalClosingPage.tsx`, `src/features/closingExports/FiscalPeriodExportSection.tsx` | Ya hay overlays separados, pero no un contrato unico de flujo para guardar, revisar, exportar y cerrar. Sprint 11 mejora la jerarquia, pero pospone la migracion para no tocar calculos ni persistencia. | medio | Disenar mini StepFlow de revision documental -> snapshot -> exportacion en sprint separado, solo cuando el workspace financiero tenga contratos mas desacoplados | Seguimiento post Sprint 11 |
