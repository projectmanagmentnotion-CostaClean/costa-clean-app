# CP-3B.2A.6R Reproducible Rebaseline V6R

Date: 2026-08-03

Status: `PREPARED_NOT_AUTHORIZED`

CP-3B.2A.5 proved that the historical mixed-byte pin for the V3 manifest could not be reconstructed honestly from the current Git-visible sources. CP-3B.2A.6R replaces the synthetic V6 runner with a real PostgreSQL QA adapter while keeping the reproducible identity model that uses:
...
- The manifest package stores the V6R artifact identities for the new package; the manifest itself is validated at runtime and is not self-referential.
- The V5 historical pin remains recorded as unrecoverable evidence, not as a dependency for V6R.

## V6R package boundary

The V6R package is the new executable contract for the future client-portal QA path. It includes:
