...
const psql = process.platform === 'win32'
  ...
  : 'psql'
const V4_AUTHORIZATION_ID = 'CP2B-V4-AUTHORIZATION-PENDING'
...
export function assertExecutionGateV4({ environment, manifest, gitHead, clean }) {
  ...
  if (environment.CP2B_V4_AUTHORIZATION_ID !== manifest.authorizationId) {
    throw new Error('v4_authorization_mismatch')
  }
  if (!environment.CP2B_V4_AUTHORIZED_HEAD
    || environment.CP2B_V4_AUTHORIZED_HEAD !== gitHead
    || !clean) {
...
export function verifyManifestV4(manifest) {
  if (manifest.version !== 4
    ...
    throw new Error('invalid_v4_manifest')
  }
...
export function planV4() {
  ...
  return {
    ...
    authorizationId: manifest?.authorizationId ?? V4_AUTHORIZATION_ID,
    stages: manifest?.expectedStages ?? [],
    requiredPrivateInputs: privateInputNames,
  }
}

export function preflightV4(environment) {
  const manifest = readManifestIfPresent()
  ...
  if (manifest) verifyManifestV4(manifest)
  return {
    gate: 'CP-2B-V4',
    mode: 'preflight',
    remoteWrites: 0,
...
async function execute(environment) {
  ...
  assertExecutionGateV4({ environment, manifest, gitHead, clean })
  assertTripleIdentityV4(environment)
...
export function assertTripleIdentityV4(environment) {
...
function run(executable, args, options = {}) {
  return runCommandV3(executable, args, {
    cwd: options.cwd ?? repoRoot,
    timeout: options.timeout ?? 30_000,
    maxBuffer: 20 * 1024 * 1024,
    environment: options.environment ?? process.env,
    redactFailure: options.redactFailure,
  }).stdout.trim()
}
...
async function main() {
  ...
  if (process.argv.includes('--preflight')) {
    process.stdout.write(`${JSON.stringify(preflightV4(process.env), null, 2)}\n`)
    return
  }
