...
const reportPath = path.join(
  repoRoot, 'qa-reports', 'private', 'client-portal',
  'cp3b2a1-local-proof-latest.json',
)
const baselineOrder = [
...
function captureFile(psql, port, filePath, variables = {}) {
  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    try {
      ...
      // Ignore psql status and assertion rows.
    }
  }
...
function assertPrestate(prestate) {
  if (
    prestate.liveRead !== 1
...
function assertRestored(prestate, restored) {
  assertPrestate(restored)
  for (const key of comparisonKeys) {
...
async function main() {
  ...
  const workDir = mkdtempSync(
    path.join(tmpdir(), `costa-clean-cp3b2a1-${randomBytes(5).toString('hex')}-`),
  )
  ...
  const runId = `CP3B2A-V2-${randomBytes(6).toString('hex').toUpperCase()}`
  const result = {
    result: 'FAIL',
  ...
  try {
    ...
    for (const migration of baselineOrder) {
      applyFile(postgres.psql, port, path.join(
        repoRoot, 'supabase', 'migrations', migration,
      ))
    }
    ...
    result.residue = captureFile(
      postgres.psql, port, precheckPath, variables,
    ).syntheticCollisions
    ...
    if (rollback.result !== 'PASS' || rollback.contractAbsent !== true) {
      fail('rollback_failed')
    }
    assertRestored(
      prestate,
      captureFile(postgres.psql, port, precheckPath, variables),
    )
    ...
    try {
      ...
      result.recoveryAttempts += 1
      if (recovered.result !== 'PASS' || recovered.contractAbsent !== true) {
        fail('simulated_recovery_failed')
      }
    }
    if (
      !simulatedFailure
      || result.applicationAttempts !== 2
      || result.recoveryAttempts !== 2
  ...
  if (result.result !== 'PASS' || !result.clusterDiscarded) {
    fail(result.error ?? 'cp3b2a1_local_proof_failed')
  }
  console.log('PASS: CP-3B.2A.1 PostgreSQL 17 disposable proof completed.')
  console.log('Apply/postcheck, transactional matrix and exact recovery passed.')
