...
[[ -n "$arming_line" && -n "$one_shot_line" && -n "$marker_upload_line" && -n "$pat_line" && "$arming_line" -lt "$marker_upload_line" && "$marker_upload_line" -lt "$pat_line" ]] || { printf 'CONTRACT_TEST=FAIL: one-shot marker ordering\n' >&2; exit 1; }

if awk '/name: Arming and recipient gate/{active=1} /name: Install isolated executor dependencies/{active=0} active && /SUPABASE_CP51F_TEMP_PAT/{bad=1} END{exit bad+0}' "$WORKFLOW"; then
  :
else
