#!/usr/bin/env bash
set -Eeuo pipefail
trap 'status=$?; printf "CONTRACT_TEST=FAIL: assertion at line %s (exit %s)\n" "$LINENO" "$status" >&2' ERR
...
POOLER_CONFIG="scripts/cp51f-pooler-config.sh"
POOLER_CONFIG_TEST="scripts/cp51f-pooler-config.synthetic.test.sh"
POOLER_RESOLVER="scripts/cp51f-resolve-pooler-host.sh"
POOLER_RESOLVER_TEST="scripts/cp51f-resolve-pooler-host.synthetic.test.sh"
...
not_contains 'vars.CP51F_POOLER_' "$WORKFLOW"
contains 'name: Resolve authoritative Session Pooler' "$WORKFLOW"
contains 'config/database/pooler' "$WORKFLOW"
contains 'curl --config - --request GET' "$WORKFLOW"
not_contains '--request PUT' "$WORKFLOW"
not_contains '--request POST' "$WORKFLOW"
not_contains '--request PATCH' "$WORKFLOW"
not_contains '--request DELETE' "$WORKFLOW"
contains 'bash scripts/cp51f-resolve-pooler-host.sh' "$WORKFLOW"
contains 'CP51F_POOLER_RESOLUTION=PASS' "$WORKFLOW"
contains 'CP51F_POOLER_RESOLUTION=FAIL' "$WORKFLOW"
contains 'CP51F_POOLER_PRIMARY_CONNECTION_STRING_TESTS=PASS' "$POOLER_RESOLVER_TEST"
contains 'scripts/cp51f-resolve-pooler-host.synthetic.test.sh' "$SMOKE"
contains 'connection_string' "$POOLER_RESOLVER"
contains 'CP51F_POOLER_HOST=%s' "$POOLER_RESOLVER"
not_contains 'config/database/pooler' "$SETUP"
contains "POOLER_HOST=\"\${CP51F_POOLER_HOST:-}\"" "$SETUP"
...
contains 'shellcheck -x scripts/cp51f-bootstrap-postgres17.sh' "$SMOKE"
...
contains 'SUPABASE_CP51F_TEMP_PAT:' "$WORKFLOW"
contains 'scripts/cp51f-resolve-pooler-host.sh' "$WORKFLOW"
...
contains 'bash scripts/cp51f-pooler-config.synthetic.test.sh' "$SMOKE"
...
contains 'shellcheck -x scripts/cp51f-bootstrap-postgres17.sh' "$SMOKE"
contains 'shellcheck -x scripts/cp51f-bootstrap-postgres17.sh scripts/cp51f-production-backup-setup.sh scripts/cp51f-pooler-config.sh scripts/cp51f-pooler-config.synthetic.test.sh' "$SMOKE"
...
pooler_resolution_line="$(line_of '- name: Resolve authoritative Session Pooler')"
...
pat_line="$(line_of '- name: Run certified production backup runner')"
[[ -n "$arming_line" && -n "$one_shot_line" && -n "$history_guard_line" && -n "$reservation_upload_line" && -n "$local_preflight_line" && -n "$pat_presence_line" && -n "$pooler_resolution_line" && -n "$marker_upload_line" && -n "$bootstrap_line" && -n "$pat_line" && "$arming_line" -lt "$history_guard_line" && "$history_guard_line" -lt "$reservation_upload_line" && "$reservation_upload_line" -lt "$bootstrap_line" && "$bootstrap_line" -lt "$local_preflight_line" && "$local_preflight_line" -lt "$pat_presence_line" && "$pat_presence_line" -lt "$pooler_resolution_line" && "$pooler_resolution_line" -lt "$marker_upload_line" && "$marker_upload_line" -lt "$pat_line" ]] || { printf 'CONTRACT_TEST=FAIL: fail-closed ordering\n' >&2; exit 1; }
