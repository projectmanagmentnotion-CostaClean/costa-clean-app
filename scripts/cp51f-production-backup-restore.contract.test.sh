...
[[ -f "$WORKFLOW" && -f "$RESTORE" && -f "$BOOTSTRAP" && -f "$SETUP" && -f "$JIT_CONTRACT" && -f "$JIT_MAPPING_CONTRACT" && -f "$JIT_PROBE" && -f "$JIT_PROBE_TEST" && -f "$JIT_PROBE_MOCK" && -f "$DIAGNOSTIC" && -f "$HTTP_DIAGNOSTIC" && -f "$HISTORY_SCHEMA" && -f "$POOLER_CONFIG" && -f "$POOLER_CONFIG_TEST" && -f "$SMOKE" ]] || { printf 'CONTRACT_TEST=FAIL\n' >&2; exit 1; }
