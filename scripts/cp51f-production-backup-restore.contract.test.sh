...
contains 'name: Create durable encrypted raw backup' "$WORKFLOW"
contains 'name: Upload durable encrypted raw backup' "$WORKFLOW"
contains 'CP51F_DURABLE_ENCRYPTED_BACKUP=PASS' "$WORKFLOW"
contains 'raw_artifact_sha256' "$WORKFLOW"
contains 'roles.sql' "$WORKFLOW"
contains 'schema.sql' "$WORKFLOW"
contains 'data.sql' "$WORKFLOW"
contains 'history_schema.sql' "$WORKFLOW"
contains 'history_data.sql' "$WORKFLOW"
contains 'manifest.json' "$WORKFLOW"
contains 'schema.restore.sql' "$RESTORE"
contains 'history_schema.restore.sql' "$RESTORE"
contains 'PORTABLE_SCHEMA_PREPARE' "$SETUP"
contains '--schema-only' "$SETUP"
contains '--no-owner' "$SETUP"
contains '--no-privileges' "$SETUP"
contains '--no-publications' "$SETUP"
contains '--no-subscriptions' "$SETUP"
