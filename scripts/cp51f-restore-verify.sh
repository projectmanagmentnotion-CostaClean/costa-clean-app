...
run_query() {
  ...
  if ! "$PSQL" -h "$PGSOCKET" -d "$database" -v ON_ERROR_STOP=1 -X -q -Atqc "$sql" >"$output" 2>"$error"; then
...
run_file() {
  ...
  if ! "$PSQL" -h "$PGSOCKET" -d "$database" -v ON_ERROR_STOP=1 -X -q -f "$file" >"$log" 2>&1; then
    fail "CP51F_RESTORE_ERROR: $phase failed"
  fi
  rm -f -- "$log"
}
