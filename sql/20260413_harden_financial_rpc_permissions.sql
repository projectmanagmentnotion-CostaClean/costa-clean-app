-- Harden financial write RPCs so the browser uses a narrow authenticated write boundary.
-- This migration does not modify or delete existing CostaClean data.
...
begin
  ...
  if exists (
    ...
    raise exception 'Las lineas del presupuesto contienen importes no validos.';
  end if;
...
revoke execute on function public.require_authenticated_financial_write() from public, anon;
revoke execute on function public.save_quote_with_lines(jsonb, jsonb) from public, anon;
revoke execute on function public.save_invoice_with_lines(jsonb, jsonb) from public, anon;
revoke execute on function public.refresh_invoice_payment_status(text) from public, anon;
revoke execute on function public.save_payment_and_refresh_invoice(jsonb) from public, anon;
revoke execute on function public.update_quote_status(text, text) from public, anon;
revoke execute on function public.update_invoice_status(text, text) from public, anon;
