-- Phase 1 relational UX foundation:
-- 1. Preserve manual invoice/quote concepts as entered by the user.
-- 2. Recalculate invoice paid state from payments after invoice edits.
-- 3. Prevent manual status transitions to paid outside the payment flow.
...
begin
  ...
  if p_status not in ('draft', 'issued', 'cancelled') then
    raise exception 'El estado pagado se deriva de cobros reales. Usa pagos para marcar una factura como pagada.';
  end if;
