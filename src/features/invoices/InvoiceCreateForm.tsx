...
export function InvoiceCreateForm({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!form.job_id) {
        setSubmitError('Debes seleccionar un job.')
        return
      }

      if (!form.client_id) {
        setSubmitError('No se pudo resolver el client del job.')
        return
      }
    ...
    <section className="data-section">
      ...
      {jobs.length === 0 ? (
        <div className="empty-state">
          ...
          <p>Primero debes crear al menos un job para poder facturar.</p>
        </div>
      ) : (
