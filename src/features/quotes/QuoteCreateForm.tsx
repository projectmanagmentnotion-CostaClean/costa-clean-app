...
export function QuoteCreateForm({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccessMessage('Quote creado correctamente.')
    } catch (err) {
      const message =
        err instanceof Error ? err.message : 'Error desconocido creando el quote.'
  ...
  return (
    <section className="data-section">
      <div className="section-header">
        <h2>Nuevo quote</h2>
        <p>Formulario mínimo inicial conectado a Supabase con IVA automático ({businessRules.defaultTaxRate * 100}%).</p>
      </div>

      {clients.length === 0 ? (
        <div className="empty-state">
          <strong>No hay clients disponibles</strong>
          <p>Primero debes crear al menos un client para poder crear un quote.</p>
        </div>
      ) : (
        <form className="lead-form" onSubmit={handleSubmit}>
          ...
          <label className="form-field form-field-full">
            ...
            <textarea
              ...
              placeholder="Condiciones o notas del presupuesto"
              rows={4}
            />
          </label>
          ...
          <div className="form-actions">
            <button type="submit" className="primary-button" disabled={isSubmitting}>
              {isSubmitting ? 'Guardando...' : 'Guardar quote'}
            </button>
          </div>

          {submitError ? (
            <div className="empty-state">
              <strong>No se pudo crear el quote</strong>
              <p>{submitError}</p>
            </div>
          ) : null}
