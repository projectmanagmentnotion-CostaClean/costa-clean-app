...
export function V3ClientsPage(props: V3ClientsPageProps) {
  ...
  if (selectedClient) {
    return <><V3ClientWorkspace client={selectedClient} properties={props.properties} jobs={props.jobs} quotes={props.quotes} invoices={props.invoices} payments={props.payments} recurringInvoicePlans={props.recurringInvoicePlans} balance={balanceByClient.get(selectedClient.id) ?? 0} onBack={closeClient} onEdit={() => setEditClient(selectedClient)} onCreateInvoice={() => props.onCreateInvoiceForClient(selectedClient)} onCreateQuote={() => props.onCreateQuoteForClient(selectedClient)} onOpenPropertyWorkspace={props.onOpenPropertyWorkspace} onOpenJobWorkspace={props.onOpenJobWorkspace} onOpenQuoteDetail={props.onOpenQuoteDetail} onOpenInvoiceDetail={props.onOpenInvoiceDetail} />{editClient ? <V3ClientWriteFlow existingClients={props.clients} client={editClient} onSaved={props.onClientSaved ?? (async () => undefined)} onCancel={() => setEditClient(null)} onOpenExisting={openClient} /> : null}</>
  }

  return <V3Page className="v3-clients-page">
    <V3PageTitle eyebrow="Cartera" title="Clientes" description="Contacta, revisa contexto y salta a la siguiente acción desde una lista operativa." action={<div className="v3-workspace-actions"><V3SecondaryAction onClick={() => setIsFilterOpen(true)}>Filtros</V3SecondaryAction><V3PrimaryAction onClick={() => setShowCreate(true)}>+ Nuevo cliente</V3PrimaryAction></div>} />
...
function V3ClientWorkspace({ client, properties, jobs, quotes, invoices, payments, recurringInvoicePlans, balance, onBack, onEdit, onCreateInvoice, onCreateQuote, onOpenPropertyWorkspace, onOpenJobWorkspace, onOpenQuoteDetail, onOpenInvoiceDetail }: { client: ClientListItem; properties: PropertyListItem[]; jobs: JobListItem[]; quotes: QuoteListItem[]; invoices: InvoiceListItem[]; payments: PaymentListItem[]; recurringInvoicePlans: RecurringInvoicePlanListItem[]; balance: number; onBack: () => void; onEdit: () => void; onCreateInvoice: () => void; onCreateQuote: () => void; onOpenPropertyWorkspace: (id: string) => void; onOpenJobWorkspace: (id: string) => void; onOpenQuoteDetail: (id: string) => void; onOpenInvoiceDetail: (id: string) => void }) {
  ...
  return <V3Page className="v3-client-workspace"><button type="button" className="v3-workspace-back" onClick={onBack}><V3Icon name="back" /> Clientes</button><V3PageTitle eyebrow="Ficha de cliente" title={client.full_name} description={`${client.display_code ?? 'Sin código'} · ${client.status === 'inactive' ? 'Inactivo' : 'Activo'}`} action={<V3SecondaryAction onClick={onEdit}>Editar</V3SecondaryAction>} /><div className="v3-workspace-actions"><V3ContactActions phone={client.phone} email={client.email} clientName={client.full_name} /><V3PrimaryAction onClick={onCreateInvoice}>+ Nueva factura</V3PrimaryAction><V3SecondaryAction onClick={onCreateQuote}>Nuevo presupuesto</V3SecondaryAction></div>
