...
export function V3ClientsPage(props: V3ClientsPageProps) {
  ...
  return <V3Page className="v3-clients-page">
    <V3PageTitle eyebrow="Cartera" title="Clientes" description="Contacta, revisa contexto y salta a la siguiente acción desde una lista operativa." action={<V3PrimaryAction onClick={() => setShowCreate(true)}>+ Nuevo cliente</V3PrimaryAction>} />
    <section className="v3-invoice-controls v3-crm-search" aria-label="Buscar clientes"><label className="v3-field"><span>Buscar clientes</span><input className="v3-input" type="search" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Nombre, código, teléfono o email" /></label><button type="button" className="v3-filter-trigger" onClick={() => setIsFilterOpen(true)} aria-haspopup="dialog" aria-expanded={isFilterOpen}>Filtrar <V3Icon name="chevronDown" size={14} /></button></section>
