...
export function V3ClosingPage(props: V3ClosingPageProps) {
  ...
  const invoiceBase = summary.invoices.reduce((total, invoice) => total + Number(invoice.subtotal || 0), 0)
  const invoiceVat = summary.invoices.reduce((total, invoice) => total + Number(invoice.tax_amount || 0), 0)
  const invoiceGross = summary.invoices.reduce((total, invoice) => total + Number(invoice.total || 0), 0)
  return <V3Page className="v3-closing-page"><V3PageTitle eyebrow="Control fiscal interno" title="Cierres" description="Una lectura operativa del periodo, con cifras calculadas, revisión y salidas separadas." />
    ...
    <section className="v3-closing-deterministic" data-qa="closing-deterministic-summary" aria-labelledby="v3-closing-deterministic-title"><div className="v3-closing-section-heading"><div><span className="v3-page-title__eyebrow">Fuente de datos</span><h2 id="v3-closing-deterministic-title">Resumen determinista</h2><p>Datos calculados del periodo activo. Estos importes son la referencia operativa del cierre.</p></div><V3EntityStatus context="Cifras" label="Calculadas" tone="neutral" /></div><V3KpiGroup><V3Kpi label="Facturado" value={formatCurrency(summary.deterministicSummary.totalInvoiced)} hint={periodLabel} /><V3Kpi label="Cobrado" value={formatCurrency(summary.deterministicSummary.totalCollected)} hint="Facturas emitidas en el periodo" /><V3Kpi label="Pendiente" value={formatCurrency(summary.deterministicSummary.totalOutstanding)} hint={`${summary.pendingInvoiceCount} factura(s)`} /></V3KpiGroup><div className="v3-closing-financial-breakdown" aria-label="Desglose de facturación"><div><span>Base imponible</span><strong>{formatCurrency(invoiceBase)}</strong></div><div><span>IVA</span><strong>{formatCurrency(invoiceVat)}</strong></div><div><span>Total facturado</span><strong>{formatCurrency(invoiceGross)}</strong></div></div></section>
