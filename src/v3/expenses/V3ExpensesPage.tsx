...
import { V3ActionGroup, V3EmptyState, V3EntityList, V3ErrorState, V3Kpi, V3KpiGroup, V3ListWorkspace, V3Page, V3PageTitle, V3PrimaryAction, V3Search, V3Select } from '../components/V3Primitives'
...
export function V3ExpensesPage(props: V3ExpensesPageProps) {
  ...
  const listWindow = useV3ListWindow(visibleExpenses, { resetKey: `${search}|${sort}` })
  ...
  return <V3Page className="v3-expenses-page v3-finance-page"><V3PageTitle eyebrow="Soporte y revisión" title="Gastos" description={`${props.activeFilterLabel ? `${props.activeFilterLabel} · ` : ''}Importes, soporte documental y revisión interna en una sola lectura.`} action={<V3ActionGroup><V3PrimaryAction onClick={props.onCreateExpense}>+ Nuevo gasto</V3PrimaryAction></V3ActionGroup>} /><div className="v3-module-controls"><V3Search value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Proveedor, referencia, concepto o categoría" /><V3Select aria-label="Ordenar gastos" value={sort} onChange={(event) => setSort(event.target.value as typeof sort)}><option value="recent">Más recientes</option><option value="oldest">Más antiguos</option><option value="amount">Mayor importe</option></V3Select></div><V3KpiGroup variant="supporting"><V3Kpi label="Gasto este mes" value={formatCurrency(totalThisMonth)} hint="Suma real por fecha de gasto" /><V3Kpi label="Por revisar" value={String(fiscalSummary.needsReviewCount)} hint="Estado interno de revisión fiscal" /></V3KpiGroup><V3ListWorkspace label="Gastos" {...listWindow} onPageChange={listWindow.setPage}>{props.error ? <V3ErrorState title="Error cargando gastos" description={props.error} /> : null}{!props.error && visibleExpenses.length === 0 ? <V3EmptyState title="Sin gastos visibles" description="Ajusta la búsqueda o el filtro para continuar." /> : null}<V3EntityList label="Gastos">{listWindow.pageItems.map((expense) => <V3ExpenseRow key={expense.id} expense={expense} onOpen={() => { listScrollYRef.current = window.scrollY; setSelectedExpenseId(expense.id); props.onOpenExpenseDeepLink(expense.id); window.requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: 'auto' })) }} />)}</V3EntityList></V3ListWorkspace></V3Page>
}
