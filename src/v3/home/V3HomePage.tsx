...
export function V3HomePage({ invoices, payments, expenses, jobs, quotes, alerts, operationalIncidents, onRunKpiAction, onOpenAlert, onRunOperationalAction, onOpenAllAlerts }: V3HomePageProps) {
  ...
  const operationalKpis = [
    model.operational.scheduledJobs > 0 ? <V3Kpi key="scheduled" label="Servicios programados" value={String(model.operational.scheduledJobs)} /> : null,
    model.operational.completedJobs > 0 ? <V3Kpi key="completed" label="Servicios completados" value={String(model.operational.completedJobs)} /> : null,
    model.operational.completedUnbilledJobs > 0 ? <V3Kpi key="unbilled" label="Completados sin factura" value={String(model.operational.completedUnbilledJobs)} onClick={() => onRunKpiAction('completed_jobs_without_invoice')} /> : null,
    model.operational.openQuotes > 0 ? <V3Kpi key="open-quotes" label="Presupuestos abiertos" value={String(model.operational.openQuotes)} onClick={() => onRunKpiAction('open_quotes')} /> : null,
    model.operational.acceptedQuotes > 0 ? <V3Kpi key="accepted-quotes" label="Presupuestos aceptados" value={String(model.operational.acceptedQuotes)} /> : null,
    model.operational.overdueInvoices > 0 ? <V3Kpi key="overdue" label="Facturas vencidas" value={String(model.operational.overdueInvoices)} /> : null,
    model.operational.expensesWithoutSupport > 0 ? <V3Kpi key="missing-support" label="Gastos sin soporte" value={String(model.operational.expensesWithoutSupport)} onClick={() => onRunKpiAction('expenses_without_receipt')} /> : null,
    model.operational.alerts > 0 ? <V3Kpi key="alerts" label="Alertas activas" value={String(model.operational.alerts)} /> : null,
  ].filter(Boolean)
  return <V3Page className="v3-home-page"><V3PageTitle eyebrow="Resumen ejecutivo" title="Negocio hoy" description="Una lectura real de facturación, cobros, gastos y próximos asuntos." action={<V3ActionGroup><V3Select aria-label="Periodo del dashboard" value={`${periodKind}:${periodKey}`} onChange={({ currentTarget }) => { const [kind, key] = currentTarget.value.split(':') as [DashboardPeriodKind, string]; setPeriodKind(kind); setPeriodKey(key) }}><option value={`month:${currentKey}`}>Este mes</option><option value={`quarter:${now.getFullYear()}-Q${Math.floor(now.getMonth() / 3) + 1}`}>Este trimestre</option><option value={`year:${now.getFullYear()}`}>Este año</option></V3Select></V3ActionGroup>} /><V3KpiGroup><V3Kpi label="Facturado" value={format(model.invoiced)} hint={growthText(model.growth.invoiced)} onClick={() => onRunKpiAction('invoiced_this_month')} /><V3Kpi label="Cobrado" value={format(model.collected)} hint={`${growthText(model.growth.collected)} · facturas emitidas en el periodo`} onClick={() => onRunKpiAction('collected_this_month')} /><V3Kpi label="Pendiente de cobro" value={format(model.outstanding)} hint="Facturas emitidas en el periodo" onClick={() => onRunKpiAction('outstanding_invoices')} /><V3Kpi label="Gastos" value={format(model.expenses)} hint={growthText(model.growth.expenses)} onClick={() => onRunKpiAction('expenses_this_month')} /><V3Kpi label="Resultado estimado" value={format(model.estimatedResult)} hint="Facturado − gastos" /><V3Kpi label="IVA estimado" value={format(model.estimatedVat)} hint="Estimación: repercutido − soportado elegible" /></V3KpiGroup><V3Section label="Evolución del negocio"><V3RevenueTrend points={model.trend} /><p className="v3-section-copy">Periodo: {model.period.label}. El IVA es una estimación operativa y no sustituye la liquidación oficial.</p></V3Section>{operationalKpis.length > 0 ? <V3Section label="Operativa del periodo"><div className="v3-dashboard-operational">{operationalKpis}</div></V3Section> : null}<V3HomePriorityQueue label="Atención del periodo" priorities={periodPriorities} onOpenAlert={onOpenAlert} onRunIncident={(incident) => onRunOperationalAction(incident.primaryAction)} onRunKpiAction={onRunKpiAction} /><V3HomePriorityQueue label="Estado actual" priorities={priorities} onOpenAlert={onOpenAlert} onRunIncident={(incident) => onRunOperationalAction(incident.primaryAction)} onRunKpiAction={onRunKpiAction} onOpenAll={onOpenAllAlerts} /><V3SecondaryAction onClick={() => onRunKpiAction('invoiced_this_month')}>Ver detalle financiero</V3SecondaryAction></V3Page>
}
