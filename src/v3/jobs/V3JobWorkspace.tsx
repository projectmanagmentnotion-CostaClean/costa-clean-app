...
export function V3JobWorkspace(props: V3JobWorkspaceProps) {
  ...
  return <V3Page className="v3-job-workspace">
    ...
    <V3DetailSection title="Situación"><div className="v3-summary"><div><span>Estado</span><V3EntityStatus label={operational.label} tone={operational.state === 'completed' ? 'success' : operational.state === 'cancelled' ? 'danger' : operational.state === 'review' ? 'warning' : 'neutral'} /></div><div><span>Facturación</span><V3EntityStatus label={invoice ? paymentSummary ? getInvoiceFinancialStatusLabel(paymentSummary.financialStatus) : 'Facturada' : 'Sin facturar'} tone={invoice && paymentSummary?.financialStatus === 'paid' ? 'success' : invoice ? 'warning' : 'neutral'} /></div><div><span>Importe</span><strong>{invoice ? formatCurrency(invoice.total) : 'Pendiente de factura'}</strong></div></div></V3DetailSection>
