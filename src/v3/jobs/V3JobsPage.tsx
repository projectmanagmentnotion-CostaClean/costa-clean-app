...
import { V3EntityList, V3Kpi, V3KpiGroup, V3ListWorkspace, V3Page, V3PageTitle, V3PrimaryAction, V3Search, V3SecondaryAction, V3TabStrip } from '../components/V3Primitives'
...
interface V3JobsPageProps { jobs: JobListItem[]; clients: ClientListItem[]; properties: PropertyListItem[]; quotes: QuoteListItem[]; invoices: InvoiceListItem[]; payments: PaymentListItem[]; error: string | null; initialJobId?: string | null; duplicateCount?: number; onReviewDuplicates?: () => void; onCreateJob: () => void; onRefresh: () => Promise<void>; onOpenClient: (id: string) => void; onOpenProperty: (id: string) => void; onOpenQuote: (id: string) => void; onOpenInvoice: (id: string) => void; onCreateInvoice: (job: JobListItem) => void; activeFilter?: JobModuleFilter | null; activeFilterLabel?: string | null }

function getInitialJobFilter(activeFilter: JobModuleFilter | null | undefined): JobFilter {
  if (activeFilter === 'completed_without_invoice' || activeFilter === 'completed_without_invoice_2d') return 'completed'
  if (activeFilter === 'tomorrow' || activeFilter === 'upcoming' || activeFilter === 'scheduled') return 'upcoming'
  return 'today'
}
...
export function V3JobsPage(props: V3JobsPageProps) {
  ...
  const [filter, setFilter] = useState<JobFilter>(() => getInitialJobFilter(props.activeFilter))
  ...
  const listWindow = useV3ListWindow(visibleJobs, { resetKey: `${filter}|${search}` })
  ...
  return <V3Page className="v3-jobs-page">
    ...
    <V3ListWorkspace label="Servicios" {...listWindow} onPageChange={listWindow.setPage}>
      ...
      <V3EntityList label="Servicios">{listWindow.pageItems.map((job) => <V3JobRow key={job.id} job={job} invoice={invoiceByJob.get(job.id) ?? null} today={today} onOpen={() => openJob(job.id)} />)}</V3EntityList>
    </V3ListWorkspace>
