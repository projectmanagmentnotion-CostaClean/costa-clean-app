...
export function V3LeadWorkspace({ lead, draft, quotes, client, onBack, onRefresh, onOpenQuote, onOpenClient }: V3LeadWorkspaceProps) {
  ...
  return <V3Page className="v3-lead-workspace">
    ...
    {isEditOpen ? <V3StepFlow title="Editar lead" onCancel={() => setIsEditOpen(false)} onComplete={saveEdit} error={feedback} busy={isBusy} completeLabel="Guardar cambios" steps={[{ id: 'contact', title: 'Contacto', content: <><label className="v3-field"><span>Nombre completo</span><V3Input autoFocus value={form.full_name} onChange={(event) => setForm({ ...form, full_name: event.target.value })} /></label><label className="v3-field"><span>Teléfono</span><V3Input value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} /></label><label className="v3-field"><span>Ciudad</span><V3Input value={form.city} onChange={(event) => setForm({ ...form, city: event.target.value })} /></label></> }, { id: 'status', title: 'Estado', content: <label className="v3-field"><span>Estado</span><select className="v3-input" value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value })}><option value="new">Nuevo</option><option value="contacted">Contactado</option><option value="quoted">Presupuestado</option><option value="won">Ganado</option><option value="lost">Perdido</option></select></label> }, { id: 'review', title: 'Revisión', content: <V3Textarea aria-label="Notas" placeholder="Notas del lead" /> }]} /> : null}
  </V3Page>
}
