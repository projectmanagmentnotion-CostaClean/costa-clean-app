...
export function V3PropertyWorkspace({ property, clients, jobs, quotes, invoices, payments, activeTab, onTabChange, onBack, onEdit, onOpenClient, onOpenJob, onOpenQuote, onOpenInvoice, onOpenPayment, onCreateJob, onCreateQuote, onCreateInvoice }: { property: PropertyListItem; clients: ClientListItem[]; jobs: JobListItem[]; quotes: QuoteListItem[]; invoices: InvoiceListItem[]; payments: PaymentListItem[]; activeTab: PropertyWorkspaceTab; onTabChange: (tab: PropertyWorkspaceTab) => void; onBack: () => void; onEdit: () => void; onOpenClient: (id: string) => void; onOpenJob: (id: string) => void; onOpenQuote: (id: string) => void; onOpenInvoice: (id: string) => void; onOpenPayment: (id: string) => void; onCreateJob: () => void; onCreateQuote: () => void; onCreateInvoice: () => void }) {
  ...
  return <V3Page className="v3-property-workspace"><button type="button" className="v3-workspace-back" onClick={onBack}>← Inmuebles</button><V3PageTitle eyebrow={property.display_code ?? property.id} title={property.name} description={`${getPropertyTypeLabel(property.property_type)} · ${property.address}`} action={<V3SecondaryAction onClick={onEdit}>Editar</V3SecondaryAction>} /><div className="v3-filter-tabs" role="tablist" aria-label="Secciones del inmueble">{tabs.map(([tab, label]) => <button key={tab} type="button" role="tab" aria-selected={activeTab === tab} className={activeTab === tab ? 'is-active' : ''} onClick={() => onTabChange(tab)}>{label}</button>)}</div>
    ...
    {activeTab === 'payments' ? <RelationSection label="Cobros" items={relatedPayments.map((item) => ({ id: item.id, title: item.invoice_display_code ?? item.invoice_id, detail: `${formatCurrency(item.amount)} · ${formatDateEs(item.payment_date)}`, onClick: () => onOpenPayment(item.id) }))} /> : null}
