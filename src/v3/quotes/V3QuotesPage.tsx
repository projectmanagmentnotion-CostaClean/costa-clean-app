...
export function V3QuotesPage(props: V3QuotesPageProps) {
  ...
  return <V3Page className="v3-quotes-page">
    ...
    <div className="v3-entity-list" role="list" aria-label="Presupuestos">{visibleQuotes.map((quote) => <V3QuoteRow key={quote.id} quote={quote} clients={props.clients} onOpen={() => openQuote(quote.id)} onDownload={() => props.onDownloadQuote(quote)} onConvert={() => void convertQuote(quote)} isBusy={busyQuoteId === quote.id} canConvert={canConvertQuoteToInvoice(quote, props.invoices)} />)}</div>
  </V3Page>
}

function V3QuoteRow({ quote, clients, onOpen, onDownload, onConvert, isBusy, canConvert }: { quote: QuoteListItem; clients: ClientListItem[]; onOpen: () => void; onDownload: () => void; onConvert: () => void; isBusy: boolean; canConvert: boolean }) {
  const label = formatQuoteLabel({ ...quote, client_name: clients.find((client) => client.id === quote.client_id)?.full_name ?? null })
  return <V3EntityListItem onClick={onOpen} ariaLabel={`Abrir presupuesto ${label}`}><div className="v3-quote-row__main"><strong>{quote.display_code ?? quote.id}</strong><span>{formatClientLabel(quote)} · {quote.notes?.trim() || 'Presupuesto de servicios'}</span><small>{quote.created_at ? formatDateEs(quote.created_at) : 'Fecha no disponible'}</small></div><div className="v3-quote-row__side"><strong>{formatCurrency(quote.total)}</strong><V3Status label={getStatusLabel(quote.status)} tone={statusTone(quote.status)} /></div><div className="v3-quote-row__actions"><V3SecondaryAction onClick={(event) => { event.stopPropagation(); onDownload() }} ariaLabel={`Descargar presupuesto ${quote.display_code ?? quote.id}`}>Descargar</V3SecondaryAction>{canConvert ? <V3PrimaryAction onClick={(event) => { event.stopPropagation(); onConvert() }} disabled={isBusy} ariaLabel={`Convertir presupuesto ${quote.display_code ?? quote.id} en factura`}>{isBusy ? 'Convirtiendo…' : 'Facturar'}</V3PrimaryAction> : null}</div></V3EntityListItem>
}

function V3QuoteWorkspace({ quote, clients, properties, jobs, invoices, busy, onBack, onDownload, onShare, onConvert, onOpenClientWorkspace, onOpenPropertyWorkspace, onOpenJobWorkspace, onOpenInvoiceDetail }: { quote: QuoteListItem; clients: ClientListItem[]; properties: PropertyListItem[]; jobs: JobListItem[]; invoices: InvoiceListItem[]; busy: boolean; onBack: () => void; onDownload: () => void; onShare: () => void; onConvert: () => void; onOpenClientWorkspace: (id: string) => void; onOpenPropertyWorkspace: (id: string) => void; onOpenJobWorkspace: (id: string) => void; onOpenInvoiceDetail: (id: string) => void }) {
  ...
  return <V3Page className="v3-quote-workspace"><button type="button" className="v3-workspace-back" onClick={onBack}>← Presupuestos</button><V3PageTitle eyebrow={quote.display_code ?? quote.id} title={getStatusLabel(quote.status)} description={`${client?.full_name ?? formatClientLabel(quote)} · ${quote.created_at ? formatDateEs(quote.created_at) : 'Fecha no disponible'}`} /><div className="v3-workspace-total"><strong>{formatCurrency(quote.total)}</strong><span>Base {formatCurrency(quote.subtotal)} · IVA {formatCurrency(quote.tax_amount ?? 0)}</span></div><div className="v3-workspace-actions">{convertible ? <V3PrimaryAction onClick={onConvert} disabled={busy} ariaLabel={`Convertir presupuesto ${quote.display_code ?? quote.id} en factura`}>{busy ? 'Convirtiendo…' : 'Convertir en factura'}</V3PrimaryAction> : null}<V3SecondaryAction onClick={onDownload} ariaLabel={`Descargar presupuesto ${quote.display_code ?? quote.id}`}>Descargar PDF</V3SecondaryAction><V3SecondaryAction onClick={onShare} ariaLabel={`Compartir presupuesto ${quote.display_code ?? quote.id}`}>Compartir</V3SecondaryAction></div>
    ...
    <V3Section label="Origen / relaciones"><div className="v3-related-links">{job ? <button type="button" className="v3-inline-link" onClick={() => onOpenJobWorkspace(job.id)}>Servicio {job.display_code ?? job.id}</button> : <span className="v3-section-copy">Sin servicio asociado.</span>}{invoice ? <button type="button" className="v3-inline-link" onClick={() => onOpenInvoiceDetail(invoice.id)}>Factura {invoice.display_code ?? invoice.invoice_number ?? invoice.id}</button> : <span className="v3-section-copy">Sin factura vinculada.</span>}</div></V3Section>
