...
function V3RecurringPlanFlow({ client, plans, properties, quotes, initialPlan, onCancel, onSaved, onOpenExistingPlan, onDirtyChange }: V3RecurringPlanFlowProps) {
  ...
  return (
    <V3StepFlow title={initialPlan ? 'Editar plan recurrente' : 'Nuevo plan recurrente'} onCancel={requestClose} onComplete={() => submit()} error={error} busy={busy} completeLabel={initialPlan ? 'Guardar cambios' : 'Guardar plan'} validateStep={(step) => step === 0 && !form.title.trim() ? 'Debes indicar un título para el plan recurrente.' : step === 1 && !form.next_issue_date ? 'Debes indicar la próxima fecha de emisión.' : null} steps={[
      { id: 'identity', title: 'Identidad', description: 'Define el cliente y el contexto del plan.', content: <><V3Field label="Cliente"><V3Input value={`${client.display_code ?? 'Cliente'} · ${client.full_name}`} readOnly /></V3Field><V3Field label="Título"><V3Input value={form.title} onChange={(event) => updateForm('title', event.target.value)} autoFocus required /></V3Field><V3Field label="Inmueble"><V3Select value={form.property_id} onChange={(event) => updateForm('property_id', event.target.value)}><option value="">Sin inmueble fijo</option>{availableProperties.map((property) => <option key={property.id} value={property.id}>{formatPropertyLabel(property)}</option>)}</V3Select></V3Field><V3Field label="Presupuesto de referencia"><V3Select value={form.quote_id} onChange={(event) => updateForm('quote_id', event.target.value)}><option value="">Sin presupuesto fijo</option>{availableQuotes.map((quote) => <option key={quote.id} value={quote.id}>{formatQuoteLabel(quote)}</option>)}</V3Select></V3Field></> },
      { id: 'schedule', title: 'Calendario', description: 'Define la frecuencia y la próxima emisión.', content: <><V3Field label="Cadencia"><V3Select value={form.frequency} onChange={(event) => updateForm('frequency', event.target.value as RecurringInvoiceFrequency)}>{(['weekly', 'biweekly', 'monthly', 'quarterly'] as const).map((frequency) => <option key={frequency} value={frequency}>{getRecurringFrequencyLabel(frequency)}</option>)}</V3Select></V3Field><V3Field label="Próxima emisión"><V3Input type="date" value={form.next_issue_date} onChange={(event) => updateForm('next_issue_date', event.target.value)} required /></V3Field><V3Field label="Factura por defecto"><V3Select value={form.default_invoice_status} onChange={(event) => updateForm('default_invoice_status', event.target.value as RecurringInvoicePlanInvoiceStatus)}><option value="draft">Borrador</option><option value="issued">Emitida</option></V3Select></V3Field><V3Field label="IVA (%)"><V3Input inputMode="decimal" value={form.tax_rate} onChange={(event) => updateForm('tax_rate', event.target.value)} /></V3Field></> },
      ...lines.map((line, index) => ({ id: `line-${line.local_id}`, title: `Línea ${index + 1}`, description: 'Una línea por paso mantiene el contenido dentro del viewport.', content: <><V3Field label="Concepto"><V3Input value={line.concept} onChange={(event) => updateLine(line.local_id, 'concept', event.target.value)} required autoFocus /></V3Field><V3Field label="Cantidad"><V3Input value={line.quantity} onChange={(event) => updateLine(line.local_id, 'quantity', event.target.value)} required /></V3Field><V3Field label="Unidad"><V3Input value={line.unit} onChange={(event) => updateLine(line.local_id, 'unit', event.target.value)} required /></V3Field><V3Field label="Precio"><V3Input inputMode="decimal" value={line.unit_price} onChange={(event) => updateLine(line.local_id, 'unit_price', event.target.value)} required /></V3Field><p className="v3-inline-message">Importe: {formatQuoteLineSubtotalInput(line)}</p></> })),
      { id: 'review', title: 'Revisión', description: 'Confirma el total y las notas antes de guardar.', content: <><p className="v3-inline-message">Total estimado: {formatCurrency(roundMoney(subtotal + taxAmount))}</p><V3Field label="Notas visibles"><V3Textarea value={form.notes} onChange={(event) => updateForm('notes', event.target.value)} /></V3Field><V3Field label="Notas internas"><V3Textarea value={form.internal_notes} onChange={(event) => updateForm('internal_notes', event.target.value)} /></V3Field></> },
    ]} />
  )
}
