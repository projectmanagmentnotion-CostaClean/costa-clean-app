...
describe('V3 dedicated shell', () => {
  it('renders the primary navigation and More entry without the legacy shell tree', () => {
    ...
    expect(html).toContain('data-brand-lockup="CostaClean"')
    expect(html).toContain('v3-top-bar__screen-label')
    expect((html.match(/data-brand-lockup="CostaClean"/g) ?? [])).toHaveLength(1)
    expect(html.indexOf('data-brand-lockup="CostaClean"')).toBeLessThan(html.indexOf('v3-top-bar__screen-label'))
