...
as $$
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'status', j.status
  ) order by j.scheduled_date desc, j.display_code desc), '[]'::jsonb)
  from (
    select *
    from public.jobs
    ...
    order by scheduled_date desc, display_code desc
...
begin
  ...
  insert into public.client_service_requests (
    ...
    nullif(btrim(p_preferred_time_window), ''), nullif(btrim(p_notes), ''),
  ...
  if v_request.id is null then
    ...
    if v_existing.id is null
      or v_existing.service_type is distinct from p_service_type
      or v_existing.preferred_date is distinct from p_preferred_date
      or v_existing.preferred_time_window is distinct from nullif(btrim(p_preferred_time_window), '')
      or coalesce(v_existing.notes, '') is distinct from coalesce(nullif(btrim(p_notes), ''), '')
...
revoke all on function public.portal_submit_service_request_v2(text, text, text, date, text, text, uuid) from public, anon;
...
grant execute on function public.portal_submit_service_request_v2(text, text, text, date, text, text, uuid) to authenticated;
...
alter function public.portal_submit_service_request_v2(text, text, text, date, text, text, uuid) owner to postgres;
