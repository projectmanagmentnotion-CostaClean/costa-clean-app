...
create or replace function portal_private.resolve_property_public_ref_v2(
  p_client_id text,
  p_property_id text
)
returns text
...
as $$
  select p.display_code
  from public.properties as p
  where p.client_id = portal_private.current_portal_client_id(p_client_id)
    and p.id = p_property_id
    and p.deleted_at is null
    and p.archived_at is null
    and p.display_code is not null
  limit 1;
$$;

create or replace function portal_private.resolve_property_id_by_public_ref_v2(
  p_client_id text,
...
as $$
  ...
  from public.properties as p
  where p.client_id = portal_private.current_portal_client_id(p_client_id)
    and p.display_code = p_property_public_ref
    and p.deleted_at is null
    and p.archived_at is null
  limit 1;
$$;
...
as $$
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'propertyPublicRef', portal_private.resolve_property_public_ref_v2(j.client_id, j.property_id),
    'propertyName', p.name,
  ...
  join public.properties as p
   ...
   and p.deleted_at is null
   and p.archived_at is null
   and portal_private.resolve_property_public_ref_v2(j.client_id, j.property_id) is not null;
$$;
...
begin
  ...
  select
    j.public_reference,
    portal_private.resolve_property_public_ref_v2(j.client_id, j.property_id) as property_public_ref,
    p.name,
  ...
  return jsonb_build_object(
    ...
    'propertyPublicRef', v_service.property_public_ref,
...
begin
  ...
  return coalesce((
    select jsonb_agg(jsonb_build_object(
      ...
      'propertyPublicRef', portal_private.resolve_property_public_ref_v2(r.client_id, r.property_id),
      'propertyName', p.name,
      'addressLabel', concat_ws(' · ', p.address, p.city),
    ...
    join public.properties as p
     ...
     and p.deleted_at is null
     and p.archived_at is null
     and portal_private.resolve_property_public_ref_v2(r.client_id, r.property_id) is not null
  ), '[]'::jsonb);
end;
$$;
...
begin
  ...
  select
    r.public_reference,
    portal_private.resolve_property_public_ref_v2(r.client_id, r.property_id) as property_public_ref,
    p.name,
    concat_ws(' · ', p.address, p.city) as address_label,
    r.service_type,
    r.preferred_date,
  ...
  return jsonb_build_object(
    ...
    'propertyPublicRef', v_request.property_public_ref,
...
begin
  ...
  where client_id = portal_private.current_portal_client_id(p_client_id)
    and id = portal_private.resolve_property_id_by_public_ref_v2(p_client_id, p_property_public_ref)
    and status = 'active'
    and deleted_at is null
    and archived_at is null;
  ...
  if v_existing.id is not null then
    ...
    return jsonb_build_object(
      ...
      'propertyPublicRef', portal_private.resolve_property_public_ref_v2(p_client_id, v_property.id),
...
alter function portal_private.resolve_property_public_ref_v2(text, text) owner to postgres;
alter function portal_private.resolve_property_id_by_public_ref_v2(text, text) owner to postgres;
