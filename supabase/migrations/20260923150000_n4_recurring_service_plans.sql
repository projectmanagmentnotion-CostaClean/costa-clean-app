...
begin
  ...
  if not exists (select 1 from public.recurring_service_plans where id = p_plan_id) then
    raise exception 'recurring_plan_not_found';
  end if;
  if jsonb_typeof(p_slots) <> 'array' or jsonb_array_length(p_slots) = 0 then
    raise exception 'recurring_slots_empty';
  end if;
  if exists (select 1 from jsonb_to_recordset(p_slots) s(weekday integer, duration_minutes integer, workers_required integer)
    where s.weekday is null or s.weekday < 1 or s.weekday > 7
      ...
      or s.workers_required is null or s.workers_required < 1 or s.workers_required > 100) then
    raise exception 'recurring_slot_invalid';
  end if;
  if (select count(*) from jsonb_to_recordset(p_slots) s(weekday integer))
      <> (select count(distinct s.weekday) from jsonb_to_recordset(p_slots) s(weekday integer)) then
    raise exception 'recurring_slot_duplicate_weekday';
  end if;
...
begin
  ...
  foreach v_date in array v_dates loop
    select s.* into v_slot
      from public.recurring_service_plan_slots s
     where s.recurring_service_plan_id = v_plan.id
       and s.weekday = extract(isodow from v_date)::integer;
    if not found then
      select p.default_start_time as start_time, p.default_duration_minutes as duration_minutes, p.default_workers_required as workers_required
        into v_slot
        from public.recurring_service_plans p
       where p.id = v_plan.id;
      if v_slot.duration_minutes is null then
        continue;
      end if;
    end if;
    insert into public.recurring_service_occurrences(
      recurring_service_plan_id, occurrence_date, status, start_time, duration_minutes, workers_required, metadata, updated_at
    ) values (
      v_plan.id, v_date, 'planned', v_slot.start_time, v_slot.duration_minutes, v_slot.workers_required,
      jsonb_build_object('timezone', coalesce(v_plan.timezone, 'Europe/Madrid')), now()
    ) on conflict (recurring_service_plan_id, occurrence_date) do nothing;
    select * into v_occurrence from public.recurring_service_occurrences
     where recurring_service_plan_id = v_plan.id and occurrence_date = v_date
     for update;
    if v_occurrence.status = 'skipped' or v_occurrence.job_id is not null then
      continue;
    end if;
    ...
    values (v_job_id, v_plan.client_id, v_plan.property_id, null, v_date, 'scheduled', v_plan.service_type, v_plan.billing_concept, v_plan.billing_quantity, v_plan.billing_unit, v_plan.billing_unit_price, v_plan.notes, v_plan.id, v_date,
      jsonb_build_object('source', 'recurring_service', 'recurring_service_plan_id', v_plan.id, 'occurrence_date', v_date, 'timezone', coalesce(v_plan.timezone, 'Europe/Madrid'), 'start_time', v_slot.start_time, 'duration_minutes', v_slot.duration_minutes, 'workers_required', v_slot.workers_required))
    ...
    if v_job_id is not null then
      ...
      update public.recurring_service_occurrences
         set status = 'generated', job_id = v_job_id, updated_at = now()
       where recurring_service_plan_id = v_plan.id and occurrence_date = v_date;
...
create or replace function public.set_recurring_service_occurrence(
  p_plan_id text, p_occurrence_date date, p_status text, p_patch jsonb default '{}'::jsonb
)
returns jsonb language plpgsql security definer set search_path = public, pg_temp as $$
declare
...
begin
  perform public.require_authenticated_financial_write();
  select * into v_occurrence from public.recurring_service_occurrences
   where recurring_service_plan_id = p_plan_id and occurrence_date = p_occurrence_date for update;
  ...
  if v_occurrence.job_id is not null then
    ...
    if found and v_job.status in ('completed', 'in_progress') then
      raise exception 'recurring_occurrence_started_immutable';
    end if;
  end if;
  ...
  update public.recurring_service_occurrences
     set status = p_status,
         ...
         workers_required = case when p_patch ? 'workers_required' then (p_patch ->> 'workers_required')::integer else workers_required end,
         updated_at = now()
   where recurring_service_plan_id = p_plan_id and occurrence_date = p_occurrence_date
