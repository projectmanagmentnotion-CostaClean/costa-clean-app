...
create or replace function public.get_job_final_profitability_base(p_job_id text)
returns jsonb language plpgsql security definer set search_path = pg_catalog, public, portal_private, pg_temp as $$
...
returns jsonb language sql security definer set search_path = pg_catalog, public, portal_private, pg_temp as $$
  select public.get_job_final_profitability_base(p_job_id);
$$;
...
revoke all on function public.save_job_expense_allocation(jsonb), public.remove_job_expense_allocation(text), public.list_job_expense_allocations(text), public.get_expense_allocation_summary(uuid), public.get_job_final_profitability_base(text), public.get_job_final_profitability(text), public.list_job_final_profitability(date, date, text, text) from public, anon;
grant execute on function public.save_job_expense_allocation(jsonb), public.remove_job_expense_allocation(text), public.list_job_expense_allocations(text), public.get_expense_allocation_summary(uuid), public.get_job_final_profitability_base(text), public.get_job_final_profitability(text), public.list_job_final_profitability(date, date, text, text) to authenticated;
