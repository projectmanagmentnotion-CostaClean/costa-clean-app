-- N9 recurring operational templates and planned-cost forecasts.
...
returns jsonb language sql security definer set search_path = pg_catalog, public, portal_private, pg_temp as $$
  select coalesce(jsonb_agg(jsonb_build_object('team_member_id', a.team_member_id, 'worker_name', m.full_name, 'planned_hours', round(sum(a.planned_minutes) / 60.0, 2)) order by m.full_name), '[]'::jsonb) from public.job_team_assignments a join public.jobs j on j.id = a.job_id join public.team_members m on m.id = a.team_member_id where j.scheduled_date between current_date and p_through_date and j.status <> 'cancelled' and j.archived_at is null and j.deleted_at is null and a.status <> 'cancelled' and portal_private.is_active_internal_staff(auth.uid()) group by a.team_member_id, m.full_name;
$$;
