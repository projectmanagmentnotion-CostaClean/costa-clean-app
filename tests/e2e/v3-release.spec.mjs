...
test('V3-8 P2-1 payment flow syncs the selected invoice outstanding balance', async () => {
  ...
  try {
    ...
    const paymentDialog = page.getByRole('dialog', { name: 'Registrar cobro' })
    await expect(paymentDialog).toBeVisible()

    const invoiceSelect = paymentDialog.getByRole('combobox', { name: 'Factura' })
    ...
    const pendingText = await paymentDialog.getByText(/^Pendiente actual:/u).textContent()
    const expectedOutstanding = pendingText?.match(/([0-9.,]+)\s*€/u)?.[1]
    expect(expectedOutstanding).toBeTruthy()
    await paymentDialog.getByRole('button', { name: 'Usar pendiente', exact: true }).click()
    const amountInput = paymentDialog.getByRole('textbox', { name: 'Importe Usar pendiente' })
    await expect(amountInput).toHaveValue(expectedOutstanding.replace(',', '.'))
    await paymentDialog.getByRole('button', { name: 'Cancelar', exact: true }).click()
